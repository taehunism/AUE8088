...
# Global variable for KAIST dataset image ID mapping
kaist_img_id_map = {}

def load_kaist_img_id_map(ann_file='/home/unicon/AUE8088/utils/eval/KAIST_val-D_annotation.json'):
    """
    Load image name and ID mapping from KAIST dataset annotation file
    """
        ...
        try:
            with open(ann_file, 'r') as f:
                ...
                for img in data.get('images', []):
                    # Remove extension from image filename and map to ID
                    img_name = Path(img.get('im_name', '')).stem
                    kaist_img_id_map[img_name] = img.get('id')
            print(f"Successfully loaded {len(kaist_img_id_map)} KAIST image ID mappings.")
        except Exception as e:
            print(f"Error loading image ID mapping: {e}")
...
def save_one_json(predn, jdict, path, index, class_map):
    ...
    # Find image ID using image name
    if image_name in img_id_map:
        image_id = img_id_map[image_name]
        
    else:
        image_id = index
        print(f"Warning!: {image_name} Don't find Mapping Image name. Use Default {index}")
    ...
    for p, b in zip(predn.tolist(), box.tolist()):
        ...
        #         "image_id": image_id,  # KAIST dataset ID same image ID
        #         "category_id": class_map[int(p[5])],
        #         "bbox": [round(x, 3) for x in b],  # [x, y, width, height] 
        #         "score": round(p[4], 5),
        #     }
        # )
        jdict.append(
            {
                ...
                "image_id": int(image_id),  # 반드시 int로 변환
                "category_id": int(0),
                "bbox": [round(x, 3) for x in b],  # float로 저장
                "score": float(round(p[4], 5))
            }
        )
...
def run(
    ...
    if save_json and len(jdict):
        # Find the most recent yolov5n-rgbt folder
        train_dir = Path('runs/train')
        exp_folders = sorted([f for f in train_dir.glob('yolov5n-rgbt*') if f.is_dir()], key=lambda x: x.stat().st_mtime, reverse=True)
        if not exp_folders:
            raise FileNotFoundError('No yolov5n-rgbt folder found in runs/train')
        save_dir = exp_folders[0].resolve()  # Use the most recent folder
        pred_json = str(save_dir / f"epoch{epoch}_predictions.json")
